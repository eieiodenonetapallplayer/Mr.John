...
app.post('/api/high-scores', authenticateToken, [
    ...
    try {
        ...
        // Notify all clients of new high score
...
app.post('/api/calculate-water-cost', authenticateToken, [
    ...
    body('shower').isInt({ min: 0 }).withMessage('Shower time must be non-negative'),
    body('dishes').isInt({ min: 0 }).withMessage('Dishwashing times must be non-negative')
], async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    ...
    // Save water usage to user profile
